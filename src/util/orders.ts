...
export const buildLimitOrder = async (params: BuildLimitOrderParams, side: OrderSide): Promise<Order> => {
    ...
    const orderConfigRequest: OrderConfigRequest = {
        ...
        makerAssetAmount: new BigNumber((isBuy ? quoteTokenAmountInBaseUnits : amount).toFixed(0)),
        takerAssetAmount: new BigNumber((isBuy ? amount : quoteTokenAmountInBaseUnits).toFixed(0)),
        makerAddress: account,
        takerAddress: ZERO_ADDRESS,
...
export const createOHLVCDataset = (params: CreateOHLVCDatasetParams, side: OrderSide, baseTokenDecimal: number) => {
    ...
    const amountDecimal = tokenAmountInUnitsToBigNumber(amount, baseTokenDecimal).toString();
